...
```{r echo=FALSE}
weight_example = data.frame("ID" = rep(1:8, 2),
           "Summer" = c(round(rnorm(8, 135, 5))),
           "Winter" = c(round(rnorm(8, 140, 5)))) %>%
  mutate(ID = factor(ID)) %>%
  pivot_longer(Summer:Winter, names_to = "Season", values_to = "Weight")
weight_example %>%
...
knitr::include_graphics("figures/Ariana_Grande_Positions.png")
```
...
```{r geom_split_violin, eval=FALSE, echo=FALSE}
...
```{r geom_flat_violin, eval=FALSE, echo=FALSE}
"%||%" <- function(a, b) {
...
GeomFlatViolin <-
  ggproto("GeomFlatViolin", Geom,
          ...
          default_aes = aes(weight = 1, color = "grey20", fill = "white", size = 0.5,
